...
class Sisow_Gateway_Abstract extends WC_Payment_Gateway
{
	...
	public function __construct()
    {
        ...
        // add overboeking action for admin
        if ($this->getCode() === 'overboeking' && is_admin()) {
            add_action('woocommerce_new_order', array($this, 'admin_order'));
        }
    }
	...
	public function init_form_fields()
    {
		...
		else if($this->id == 'overboeking' || $this->id == 'ebill')	
		{
			...
			if ($this->getCode() === 'overboeking') {
                $this->form_fields['create_admin'] = array(
                    'title' => __('From admin', 'woocommerce-sisow'),
                    'type' => 'checkbox',
                    'label' => __('Create banktransfer if order is created from admin', 'woocommerce-sisow'),
                    'default' => 'no',
                );
            }
		}
    ...
    public function admin_order($order_id) {
        // option enabled?
        if ($this->get_option('create_admin') !== 'yes') {
            return;
        }

        // only enabled for overboeking
        if ($this->getCode() !== 'overboeking') {
            return;
        }
        ...
        if ($order->get_payment_method() !== 'overboeking') {
            return;
        }
	...
	public function process_payment($order_id)
    {
		...
		else
		{
            ...
            add_post_meta($order_id, '_sisow_order', 'yes');
